...
class BreedingComponent {
	...
	private function breed(Entity $partner) {
		...
		PureEntities::getInstance()->scheduleCreatureSpawn($this->entity, $this->entity->getNetworkId(), $this->entity->getLevel(), "Animal", true, $this->entity, $owner);
	}
	...
	public function increaseAge() {
		if($this->ageTickTimer >= self::AGE_TICK_DELAY) {
			...
			}elseif(!$this->isBaby() and $this->getAge() > 0){
				$newAge = $this->getAge() - $this->ageTickTimer;
				if($newAge < 0) {
					$newAge = 0;
				}
