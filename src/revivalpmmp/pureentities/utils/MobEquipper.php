	...
	public static function equipMob(BaseEntity $entity){
		if($entity instanceof IntfCanEquip){
			...
			if($wearWeapon){
				...
				PureEntities::logOutput("[MobEquipper] set $weaponItem as weapon for " . $entity, \LogLevel::DEBUG);
			}else{
			...
			if($wearArmor){
				...
				if(mt_rand(0, 100) <= $wearChances->getFullChance()){ // full armor
					...
					PureEntities::logOutput("No armor via chance selected for $entity", \LogLevel::DEBUG);
				}
			}else{
				PureEntities::logOutput("[MobEquipper] set no armor for " . $entity, \LogLevel::DEBUG);
			}
		}
	}
