...
abstract class WalkingEntity extends BaseEntity {
	...
	public function updateMove($tickDiff) {
		...
		}elseif(($target = $this->getBaseTarget()) instanceof ItemEntity and $this instanceof IntfCanEquip){ // mob equipment
			$distance = $this->distanceSquared($this->getBaseTarget());
			if($distance <= 1.5) {
	...
	protected function checkJump($dx, $dz) {
		...
		if($blockingBlock != null and !$blockingBlock->canPassThrough() and $this->getMaxJumpHeight() > 0) {
			...
			if($upperBlock->canPassThrough() && $secondUpperBlock->canPassThrough()) {
				...
				}elseif($blockingBlock instanceof StoneSlab or $blockingBlock instanceof Stair){ // on stairs entities shouldn't jump THAT high
					$this->motion->y = $this->gravity * 4;
					PureEntities::logOutput("$this: checkJump(): found slab or stair!", \LogLevel::DEBUG);
				}elseif($this->motion->y < ($this->gravity * 3.2)){ // Magic
