...
abstract class BaseEntity extends Creature {
	...
	public function updateMovement(bool $teleport = false) : void {
		if(!$this->isClosed() and $this->getLevel() !== null) {
	...
	public function targetOption(Creature $creature, float $distance) : bool {
		return $this instanceof Monster and (!($creature instanceof Player) or ($creature->isSurvival() and $creature->spawned)) and $creature->isAlive() and !$creature->isClosed() and $distance <= 81;
	}

	/**
	 * This is called while moving around. This is specially important for entities like sheep etc.
	...
	public function attack(EntityDamageEvent $source) : void {
		...
		if($this->isClosed() or $source->isCancelled() or !($source instanceof EntityDamageByEntityEvent)) {
			return;
		}
		...
		if(($this instanceof FlyingEntity) and !($this instanceof Blaze)) {
				...
				foreach($attackedBy->getLevel()->getEntities() as $entity) {
					if($entity instanceof IntfTameable and
					   ...
					   !$entity->getBaseTarget() instanceof Monster
						...
						if($this instanceof IntfTameable and $this->isTamed() and strcasecmp($this->getOwner()->getName(), $attackedBy->getName()) === 0
						) {
