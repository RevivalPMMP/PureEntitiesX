...
/**
 * Class aioSpawner (All-In-One Spawner)
 *
 * A spawning task that handles spawning all non-player entity types.
 *
 * @package revivalpmmp\pureentities\task\spawners
 */
abstract class aioSpawner {
    ...
    public function __construct(string $entityName)
    {
        ...
        $this->networkId = $this->convertNameToNetworkId($entityName);
        if ($this->networkId != -1) {
            $this->maxSpawn = PureEntities::getInstance()->getConfig()->getNested("max-spawn." . $this->entityName, 0);
            $this->probability = PureEntities::getInstance()->getConfig()->getNested("probability." . $this->entityName, 0);
            PureEntities::logOutput("aioSpawner: got " . $this->probability . "% spawn probability for " . $entityName . " spawns with a maximum number of " . $this->maxSpawn . " living entities per level", PureEntities::DEBUG);
        } else {
            PureEntities::logOutput("aioSpawner received invalid entity name: " . $this->entityName);
        }
    }
     ...
     * @return bool
     */
    private function spawnAllowedByEntityCount(Level $level): bool {
        if ($this->maxSpawn <= 0) {
            return false;
        }
        $count = 0;
     ...
     * @return bool
     */
    private function isSpawnAllowedByBlockLight(Player $player, Position $pos, int $maxBlockLight = -1, int $minBlockLight = -1) {
    ...
    private function convertNameToNetworkId(string $name) : int {
        if (isset(Data::NETWORK_IDS[strtolower($name)])) {
           return Data::NETWORK_IDS[strtolower($name)];
        }
        return -1;
    }
